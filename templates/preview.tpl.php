<div id="skinpreview">
    <div id="preview-2d">
        <p>Steve 模型的皮肤：<img id="steve" class="skin2d"
        <?php if ($data['user']->getTexture('steve') != "") {
            echo 'src = "../textures/'.$data['user']->getTexture('steve').'"/></p>';
        } else {
            echo '/> </p>';
        } ?>

        <p>Alex 模型的皮肤：<img id="alex" class="skin2d"
        <?php if ($data['user']->getTexture('alex') != "") {
            echo 'src = "../textures/'.$data['user']->getTexture('alex').'"/></p>';
        } else {
            echo '/> <span class="skin2d">未上传</span></p>';
        } ?>

        <p>披风：<img id="cape" class="skin2d"
        <?php if ($data['user']->getTexture('cape') != "") {
            echo 'src = "../textures/'.$data['user']->getTexture('cape').'"/></p>';
        } else {
            echo '/> <span class="skin2d">未上传</span></p>';
        } ?>
    </div>
</div>
<style>
    .operations { display: inline; float: right; }
    .operations > i { padding: .5em .5em; display: inline !important; }
    .operations > i:hover { color: #555; cursor: pointer; }
    .skin2d { float: right; max-height: 64px; width: 64px; font-size: 16px; }
    .box-title { width: 100%; }
    #canvas3d { margin: 0 auto; display: block; }
    #preview-2d > p { height: 64px; line-height: 64px; }
</style>
<script type="text/javascript">

</script>
<script type="text/javascript" src="../assets/libs/skin-preview/three.min.js"></script>
<script type="text/javascript" src="../assets/libs/skin-preview/three.msp.js"></script>

<?php
// Get and set texture for preview
if ($data['user']->getTexture('skin')) {
    echo "<script>var dskin ='../textures/".$data['user']->getTexture('skin')."';</script>";
} else {
    // Default Steve Skin: https://minecraft.net/images/steve.png
    $skin_steve  = "data:image/png;base64,";

    echo "<script>/* 设置默认图像 */ var dskin = '$skin_steve'; </script>";
    echo "<script>document.getElementById('steve').src = dskin; </script>";
}
echo "<script>MSP.changeSkin(dskin);</script>";
if ($data['user']->getTexture('cape')) {
    echo "<script>var dcape ='../textures/".$data['user']->getTexture('cape')."';</script>";
    echo "<script>MSP.changeCape(dcape);</script>";
} ?>
