...
var MSP = (function(global, undefined) {
    ...
    var leftLeg2Mesh, rightLeg2Mesh, leftArm2Mesh, rightArm2Mesh;
    var leftLegMesh, rightLegMesh, leftArmMesh, rightArmMesh, capeMesh;
    ...
    var onMouseMove = function(e) {
        if (isMouseDown) {
            var mouseX = e.pageX - threecanvas.offsetLeft;
            deltaMouseX = mouseX - originMouseX;
            originMouseX = mouseX;

            var mouseY = e.pageY - threecanvas.offsetTop;
    ...
    // Cape Part
    var canvas2 = global.document.createElement('canvas');
    canvas2.width = 22;
    canvas2.height = 17;
    ...
    skin_img.onload = function() {
        ...
        canvas.height = skin_img.width;

        // Erase what was on the canvas before
        context.clearRect(0, 0, skin_img.width, skin_img.width);

        // Draw the image to the canvas
        context.drawImage(skin_img, 0, 0);

        // Convert the image if need be
        if(skin_img.height == skin_img.width/2) Convert6432To6464(context,skin_img.width/64);
        FixNonVisible(context,skin_img.width/64);
        FixOverlay(context,skin_img.width/64);
    ...
    skin_img.onerror = function() {
        console.log("Failed loading " + skin.src);
    }
    var cape_img = new Image();
    cape_img.crossOrigin = '';
    cape_img.onload = function() {
        if (cape_img.width/cape_img.height == 2) {
            canvas2.width = cape_img.width/64*22;
            canvas2.height = cape_img.height/32*17;
        } else {
            canvas2.width = cape_img.width;
            canvas2.height = cape_img.width;
        }
        ...
        // Draw the image to the canvas
        context2.drawImage(cape_img, 0, 0);
    ...
    var material3 = new THREE.MeshBasicMaterial({map: capeTexture});
    ...
    function RenderSkin() {
        ...
        threecanvas.addEventListener('mousedown', function(e) {
            e.preventDefault();
            originMouseX = e.pageX - threecanvas.offsetLeft;
            originMouseY = e.pageY - threecanvas.offsetTop;
            isMouseDown = true;
        }, false);
    ...
    return {
        ...
        get2dSkinCanvas: function(type) {
            if (type == "skin")
                return canvas;
            else if (type == "cape")
                return canvas2;
        },
        toggleHead: function (checked) {
            var head = scene.getObjectByName("head", false);
            head.visible = checked;
        },
        toggleLeftArm: function (checked) {
            var leftArm = scene.getObjectByName("leftArm", false);
            leftArm.visible = checked;
        },
        toggleRightArm: function (checked) {
            var rightArm = scene.getObjectByName("rightArm", false);
            rightArm.visible = checked;
        },
        toggleBody: function (checked) {
            var body = scene.getObjectByName("body", false);
            body.visible = checked;
        },
        toggleLeftLeg: function (checked) {
            var leftLeg = scene.getObjectByName("leftLeg", false);
            leftLeg.visible = checked;
        },
        toggleRightLeg: function (checked) {
            var rightLeg = scene.getObjectByName("rightLeg", false);
            rightLeg.visible = checked;
        },
        toggleCape: function (checked) {
            var cape = scene.getObjectByName("cape", false);
            cape.visible = checked;
        },
        toggleHead2: function (checked) {
            var head2 = scene.getObjectByName("head2", false);
            head2.visible = checked;
        },
        toggleLeftArm2: function (checked) {
            var leftArm2 = scene.getObjectByName("leftArm2", false);
            leftArm2.visible = checked;
        },
        toggleRightArm2: function (checked) {
            var rightArm2 = scene.getObjectByName("rightArm2", false);
            rightArm2.visible = checked;
        },
        toggleBody2: function (checked) {
            var body2 = scene.getObjectByName("body2", false);
            body2.visible = checked;
        },
        toggleLeftLeg2: function (checked) {
            var leftLeg2 = scene.getObjectByName("leftLeg2", false);
            leftLeg2.visible = checked;
        },
        toggleRightLeg2: function (checked) {
            var rightLeg2 = scene.getObjectByName("rightLeg2", false);
            rightLeg2.visible = checked;
        },
        allowFullScreenDragChange: function (checked) {
            allowFullScreenDrag = checked;
        },
        stopPlay: function (checked) {
            speed = checked ? 0 : 1;
        },
        playQuickly: function (checked) {
            speed = checked ? 4 : 1;
        }
    };
}(this));
